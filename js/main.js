$(document).ready(function () {
    ...
    $(".services__service .services__input").on('click', function () {
        ...
        if (price != 0) {
            if ($('.props__prop[data-serv="' + serv + '"]').length > 0) {
                ...
                $('.props').append('<div data-serv="' + serv + '" data-price = "' + price + '"  data-all-price = "' + all_price + '" class = "props__prop prop">' +
                    ...
                    '<select class="select" data-serv="' + serv + '" name = "' + serv + '-count" id = "nails" class = "nails">' +
    ...
    $('.services__service .services__input').on('change', function () {
        ...
        if ($(this).attr('data-price') == 0) {
            if ($(this).prop('checked')) {
                total_price += parseInt($(this).attr('data-all-price'));
            } else {
                total_price -= parseInt($(this).attr('data-all-price'));
            }
        }
        ...
        $('.services__price-input').val(parseInt(total_price) + parseInt(sum));
    ...
    $('.phone').on('input', function () {
        val = $(this).val();
        $.post("api/client.php", {'function': 'check_sale', 'phone': val}, function (data) {
            $('.services__sale-pers span').html(data);
            $('.services__sale-pers-input').val(data);
        });
    });

    $('#connect').on('change',function () {
        if($('#connect option:selected').attr('value')=='instagram'){
            $('.connect__input').html('<input type="text" class="form__text" placeholder="введите никнейм" name="connect-var">');
        }
        if($('#connect option:selected').attr('value')=='vk'){
            $('.connect__input').html('<input type="text" class="form__text" placeholder="ссылка на вк" name="connect-var">');
        }
    })
});
